-- Makes customer registration optional for admin-dispatched trips.
-- Previously admin_dispatch_trip raised CUSTOMER_NOT_FOUND and refused to
-- create the trip at all when the phone number typed by the dispatch
-- operator didn't match a registered customer account. Per spec, that must
-- never block dispatch: the trip should still be created, broadcast to
-- every online/approved captain exactly like any other open trip (no
-- separate path), and be acceptable by any of them. `guest_customer_phone`
-- preserves the phone number the operator typed so admin/captain still have
-- a way to identify/contact whoever called in, since there is no
-- `customers` row to join for a name in that case.

alter table public.trips alter column customer_id drop not null;

alter table public.trips add column if not exists guest_customer_phone text;
...
begin
  ...
  -- No registered account for this phone number: dispatch anyway, with
  -- customer_id left null and the typed phone preserved separately. The
  -- trip enters the exact same 'searching' broadcast pool either way, so
  -- captain-side visibility/accept behavior is identical.

...
/// Full-screen map for picking a destination by tapping a point directly -
/// an alternative to [DestinationSearchScreen]'s text search, for a place
/// that isn't in the registered places/districts list or is just easier to
/// point at. Pops with a synthetic [DestinationSuggestion] built from the
/// tapped point and its reverse-geocoded address, same shape as picking a
/// real search result so every caller downstream is unaffected.
class DestinationMapPickerScreen extends StatefulWidget {
  const DestinationMapPickerScreen({super.key});
...
class _DestinationMapPickerScreenState
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('اختر الوجهة من الخريطة')),
      body: Stack(
        children: [
          Positioned.fill(
            child: RealMapWidget(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          _picked == null
                              ? 'اضغط على أي نقطة فى الخريطة لتحديد وجهتك.'
                              : (_isGeocoding
                                    ? 'جارٍ تحديد العنوان...'
                                    : _address ?? ''),
